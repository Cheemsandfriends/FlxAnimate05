package badFlxAnimate.data;

import badFlxAnimate.motion.AdjustColor;
	...
	public static function fromFilterJson(filters:Filters = null)
	{
		...
		for (filter in Reflect.fields(filters))
		{
			switch (filter)
			{
				...
				case "ACF":
					{
						var adjust:AdjustColorFilter = Reflect.field(filters, filter);
						var effect = new AdjustColor();
						effect.hue = adjust.H;
						effect.brightness = adjust.BRT;
						effect.contrast = adjust.CT;
						effect.saturation = adjust.SAT;
						bitmapFilter.push(new openfl.filters.ColorMatrixFilter(effect.calculateFinalFlatArray()));
					}
			}
		}
		return bitmapFilter;
...
abstract SymbolInstance({}) from {}
{
	...
	function set_FF(FF:Int)
	{
		return AnimationData.setFieldBool(this, ["FF", "firstFrame"], FF);
	}
